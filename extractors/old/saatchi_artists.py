...
def get_artist_info(url, driver):
    ...
    for nav_tab in about_nav_tabs:
        nav_tab.find_element(By.XPATH, './button[@data-type="nav-tab"]').click()  # Use './' to find element inside the nav_tab
...
def get_all_artists_info(bucket_name, gcs_artists_info_name, gcs_artworks_info_name):
    local_artworks_info_path = './temporary-files/' + gcs_artworks_info_name
    ...
    artists_info = []
    for artist_link in artist_links[:50]:
        print(artist_link)
        try:
    ...
    # Store the list of dictionaries as a JSON file in temporary-files folder
    print('storing data')
...
def store_string_as_file_in_gcs(bucket_name, gcs_file_name, string):
    local_file_path = './temporary-files/' + gcs_file_name
